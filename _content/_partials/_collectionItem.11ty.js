export default async function (data) {
  ...
  const highlightsStr = (highlights || [])
    .map(({ text, url: hUrl }) =>
      hUrl ? `<li class="small" style="background-color: ${data.capsuleBackgroundColor || "white"}; color: ${data.textColor || data.backgroundColor || "inherit"}">
    <a href="${hUrl}">${text}</a></li>` : `<li class="small" style="background-color: ${data.capsuleBackgroundColor || "white"}; color: ${data.textColor || data.backgroundColor || "inherit"}">${text}</li>`
    )
    .join("\n");

  return `<article class="collection-item breakout-clickable width-max center text flow; ${pagePreview ? "page-preview" : ""}">
    <header class="collection-item_header">
    <h2><a href="${url}" class="clickable h3" target="_blank" rel="noopener noreferrer" style="color: var(--color-text);">${title}</a></h2>
