...
.values-card:is(:hover, :focus-visible) {
...
.values-card.is-active .values-card__title {
  margin: 0;
}
...
;document.addEventListener('DOMContentLoaded', function () {
  var grid = document.querySelector('.values-grid');
  if (!grid) return;
  var cards = grid.querySelectorAll('.values-card');
